...
__attribute__((always_inline)) __attribute__((section(".time_critical.shift_left_511"))) static inline void shift_left_511(uint32_t *word)
{
    ...
    __asm__ volatile("ldr r1, [%[ptr], #60] \n\t" // Load word[15]
                     "lsrs r1, r1, #31 \n\t"      // Shift Right by 31. The last bit shifted out is bit 30 (bit 510). Carry Flag = bit 510!
                     "ldmia %[ptr], {r1-r8} \n\t" // Load first 8 words
                     ...
                     "adcs r7, r7 \n\t"
                     "adcs r8, r8 \n\t"
                     "bic r8, r8, #0x80000000 \n\t" // Mask out bit 511 (highest bit of word 15)
                     "stmia %[ptr], {r1-r8} \n\t"   // Store final 8 words
                     : [ptr] "+r"(p)
                     :
                     : "r1", "r2", "r3", "r4", "r5", "r6", "r7", "r8", "memory", "cc");
#else
...
__attribute__((section(".time_critical.ldpc_78_encode"))) void ldpc_78_encode(const uint8_t *info_bytes, uint16_t info_len, uint8_t *codeword_out)
{
    ...
    // Per CCSDS 131.0-B-5: parity block is 1022 bits (511 + 511) + 2 appended zero bits = 1024 bits total
    // The two trailing zero bits come from the memset(codeword_out, 0, 1020) at the start
    int out_bit_idx = 7136;

    for (int j = 0; j < 511; j++)
    {
        uint8_t p_bit = (parity0[j / 32] >> (j % 32)) & 1;
        codeword_out[out_bit_idx / 8] |= (p_bit << (7 - (out_bit_idx % 8)));
        out_bit_idx++;
    }
    for (int j = 0; j < 511; j++)
    {
        uint8_t p_bit = (parity1[j / 32] >> (j % 32)) & 1;
        codeword_out[out_bit_idx / 8] |= (p_bit << (7 - (out_bit_idx % 8)));
        out_bit_idx++;
    }
}
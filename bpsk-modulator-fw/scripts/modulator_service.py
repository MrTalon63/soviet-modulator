...
def toggle_poly_if_needed(ser, status, desired_poly):
    ...
    if curr_poly is None:
        # Toggle it once to see what it becomes
        ser.write(b"N")
        time.sleep(0.1)
        ...
        # If it's now what we want, we're done. Otherwise toggle it again.
    ...
    while True:
        try:
            ...
            if any(
                ...
                # Configure RRC filter parameters if needed
                needs_r_config = (
                    args.rrc_alpha is not None or
                    args.rrc_min_dac is not None or
                ...
                if needs_r_config:
                    ...
                    min_dac_rate = args.rrc_min_dac if args.rrc_min_dac is not None else curr_min_dac
                    ...
                    print(f"[Serial] Configuring RRC/RC filter: alpha={alpha}, min_dac_rate={min_dac_rate}, span={span}, type={rrc_type}...")
                    ser.write(f"R {alpha} {min_dac_rate} {span} {rrc_type}\n".encode("utf-8"))
                    time.sleep(0.1)
                    ser.read_all()
                    status, _ = get_status(ser)
                ...
                if args.rrc_fixed_l is not None:
                    print(f"[Serial] Setting FPGA fixed interpolation factor to {args.rrc_fixed_l}...")
                    ser.write(f"F{args.rrc_fixed_l}\n".encode("utf-8"))
                    time.sleep(0.1)
                    ser.read_all()
                    status, _ = get_status(ser)

                # Re-fetch status after settings are applied
                status, _ = get_status(ser)
            ...
            try:
                ...
                TARGET_FIFO_LEVEL = 65536  # Target 50% of the 128KB MCU FIFO (131072 bytes * 0.5)
                print(
...
def main():
    ...
    parser.add_argument(
        "--rrc-min-dac",
        type=int,
        help="RRC/RC filter minimum DAC sample rate (Hz)",
    )
    parser.add_argument(
        "--rrc-span",
        type=int,
    ...
    parser.add_argument(
        "--rrc-fixed-l",
        type=int,
        help="FPGA fixed interpolation factor L (0=Auto, or multiples of 4: 4, 8, 12...)",
    )
    parser.add_argument(
        "--qsize",
        type=int,

...
struct Si5351Cal {
  ...
  int32_t
      correction; // Crystal correction in units of 0.01 Hz (as si5351 expects)
};
...
static unsigned long current_frame_period_ms() {
  ...
  if (modulator->get_convolutional_encoding()) {
    ...
    output_symbols_per_frame =
        (uint32_t)((modulator->get_frame_size() * 8.0) / code_rate);
  } else {
...
void init_si5351_i2c() {
  Wire1.setSDA(SI5351_SDA_PIN);
  Wire1.setSCL(SI5351_SCL_PIN);
  Wire1.begin();

  // Direct the hardcoded Wire calls in the Si5351 library to Wire1 (I2C1) by
  // copying its state
  memcpy((void *)&Wire, (const void *)&Wire1, sizeof(TwoWire));
...
void apply_si5351_lo() {
  ...
  // Calculate an even integer Multisynth divider that keeps the PLL within
  // 600-900 MHz
...
void print_help() {
  ...
  Serial.println(
      "  F - Set FPGA fixed interpolation factor (0=Auto, 4, 8, 12...)");
  Serial.println(
      "  u - Binary upload mode (raw Space Packets, timeout 5s exits)");
  ...
  Serial.println("  C <ppb*10> - Set Si5351 crystal correction (0.01 Hz units, "
                 "e.g. -150)");
  ...
  Serial.println(
      "  I <vcid> - Queue idle data MPDU frame with specified VCID (0-63)");
  ...
  Serial.println("  b - Toggle Interpolation Mode (CIC vs ZOH)");
  Serial.println("  M <mode> - Set modulation mode:");
  Serial.println("       0=BPSK, 1=QPSK, 5=OQPSK");
  Serial.println("       2=IF+BPSK, 3=IF+QPSK, 7=IF+OQPSK");
  Serial.println("  d <amp> <phase> - Set digital IF (amp 0-15, phase 0/64/128/192)");
  Serial.println("  h - Print this help menu");
}

void setup() {
  // Fix coil whine & boot crashes: Force DC/DC into PWM mode BEFORE increasing
  // power draw!
  pinMode(PIN_SMPS_MODE, OUTPUT);
  digitalWrite(PIN_SMPS_MODE, HIGH); // Use hardware-agnostic macro (GPIO 23 on
                                     // Pico 1, mapped internally on Pico 2)
  delay(10);

  // vreg_disable_voltage_limit();        // Unlock higher voltages for the
  // RP2350 vreg_set_voltage(VREG_VOLTAGE_1_20); // Manually set internal core
  // voltage to 1.30V to stabilize 400 MHz delay(100); // Allow internal LDO to
  // charge up fully set_sys_clock_khz(150000, true);     // Safely apply 400
  // MHz overclock
  delay(100); // Allow hardware peripherals (Si5351) a fraction of a second to
              // cleanly power up
  ...
  Serial.begin(921600);
  rs_init(); // Initialize Reed-Solomon tables BEFORE the modulator generates
             // its OID frame!

  BPSKModulator *temp_mod = new (modulator_memory)
      BPSKModulator(MODULATING_PIN, global_sp_fifo, 500000);
  __dmb();
  modulator = temp_mod;
  if (modulator == nullptr) {
...
void process_input(char mode, const char *data) {
  ...
  if (mode == 'r') {
    ...
    int parsed = sscanf(data, "%lu %d %f %d", &L, &span, &alpha, &type);
    if (parsed >= 1) {
      if (L > 0 && (L < 4 || L > 128)) {
        Serial.println("ERROR: FPGA oversampling factor L must be between 4 "
                       "and 128 (or 0 for Auto)");
        return;
      }
      ...
      if (span != 0 && (span < 2 || span > 40)) {
        Serial.println("ERROR: FPGA filter span must be between 2 and 40 "
                       "symbols (or 0 for Auto)");
        return;
      }
      if (type != 0 && type != 1) {
        Serial.println(
            "ERROR: type must be 0 (Raised Cosine) or 1 (Root Raised Cosine)");
        return;
      }
      if (modulator != nullptr) {
        bool was_running = modulator->is_running();
        if (was_running) {
          modulator->stop();
        }
        if (!modulator->verify_fpga_spi_connection()) {
          Serial.println(
              "ERROR: FPGA SPI connection failed! Magic ID mismatch.");
          if (was_running) {
            modulator->start();
          }
          return;
        ...
        if (L == 0 || span == 0) {
          modulator->rrc_fpga_auto = true;
          uint32_t chosen_L = modulator->choose_L(
              modulator->get_symbolrate(), modulator->get_rrc_filter_span());
          uint32_t chosen_span = modulator->choose_span(alpha);
          modulator->set_rrc_fixed_L(chosen_L);
          modulator->update_rrc_config(true, alpha, (uint8_t)chosen_span,
                                       modulator->get_rrc_min_dac_rate(),
                                       type == 1);
          Serial.print("FPGA Mode Enabled (AUTO): L=");
          Serial.print(chosen_L);
          Serial.print(", Span=");
          Serial.print(chosen_span);
          Serial.print(", Alpha=");
          Serial.println(alpha);
        } else {
          modulator->rrc_fpga_auto = false;
          modulator->set_rrc_fixed_L(L);
          modulator->update_rrc_config(true, alpha, (uint8_t)span,
                                       modulator->get_rrc_min_dac_rate(),
                                       type == 1);
          Serial.print("FPGA Mode Enabled: L=");
          Serial.print(L);
          Serial.print(", Span=");
          Serial.print(span);
          Serial.print(", Alpha=");
      ...
      Serial.println(
          "ERROR: Invalid format. Expected space-separated parameters.");
    }
  } else if (mode == 'O') {
    ...
    if (modulator != nullptr) {
      modulator->set_modulation_mode(m);
      Serial.print("Modulation mode set to: 0x");
      Serial.print(m, HEX);
      Serial.print(" = ");
      switch (m) {
        case 0: Serial.println("BPSK"); break;
        case 1: Serial.println("QPSK"); break;
        case 5: Serial.println("OQPSK"); break;
        case 2: Serial.println("IF+BPSK"); break;
        case 3: Serial.println("IF+QPSK"); break;
        case 7: Serial.println("IF+OQPSK"); break;
        default: Serial.println("Unknown (raw byte)"); break;
      }
    }
  } else if (mode == 'd') {
    int amp = -1, phase = -1;
    sscanf(data, "%d %d", &amp, &phase);
    if (amp < 0 || amp > 15 || phase < 0 || phase > 192) {
      Serial.println("ERROR: Usage: d <amp 0-15> <phase 0/64/128/192>");
    } else if (modulator != nullptr) {
      modulator->set_digital_if((uint8_t)amp, (uint8_t)phase);
      Serial.print("Digital IF set: amplitude=");
      Serial.print(amp);
      Serial.print(", phase=");
...
void __not_in_flash_func(loop1)() {
  static bool printed_loop1_started = false;
  if (!printed_loop1_started) {
    printed_loop1_started = true;
  }
  if (modulator == nullptr)
    return;

  // Yield completely if modulator is not running or settings are actively being
  // changed
  ...
  // If we have user data, pack it. If not, dynamically generate an OID frame
  // (VCID 63) so the frame counter increments, avalanching the parity to
  // prevent static false-syncs!
  if (modulator->can_queue_binary_frame()) {
    uint32_t t0 = time_us_32();
    if (modulator->has_data_to_send()) {
      modulator->generate_and_queue_mpdu();
    } else {
      modulator->generate_and_queue_idle_frame(
          0x3F); // Dynamically encode VCID 63
    }
...
void loop() {
  ...
  if (binary_upload_mode &&
      (now - last_telemetry >=
       200)) { // 5Hz to provide flow control feedback with minimal overhead

    static float cached_temp = 0.0f;
    static unsigned long last_temp_read = 0;
    if (now - last_temp_read >= 5000 || cached_temp == 0.0f) {
      cached_temp = analogReadTemp(); // Caching temperature read to avoid
                                      // frequent synchronous ADC blocks
      last_temp_read = now;
    }
    ...
    uint32_t frame_tx_time_us = (uint32_t)((symbols_per_frame * 1000000ULL) /
                                           modulator->get_symbolrate());
    uint32_t cpu_time_us = modulator->perf_mpdu_us + modulator->perf_bb_us;
    float headroom_pct =
        (1.0f - ((float)cpu_time_us / (float)frame_tx_time_us)) * 100.0f;
  ...
  if (Serial.available()) {
    if (binary_upload_mode) {
      while (Serial.available()) {
        if (modulator->get_sp_fifo_free() > 0) {
          ...
          last_binary_rx_ms =
              now; // Only reset watchdog when we actually pull data
        } else {
    ...
    if (c == 'u') {
      ...
      modulator->clear_sp_fifo(); // Safely reset parsing state WITHOUT
                                  // violently aborting the DMA!
    ...
    if (waiting_for_input) {
      ...
      // Ignore line endings and whitespace chatter from terminal when NOT in
      // input mode
      if (c == '\r' || c == '\n' || c == ' ') {
        return;
      }
      ...
      case 'k': {
        Serial.println(
            "Enter puncturing rate (0=1/2, 1=2/3, 2=3/4, 3=5/6, 4=7/8):");
        waiting_for_input = true;
        input_mode = 'k';
        input_idx = 0;
        break;
      ...
      case 'F': {
        Serial.println("Enter FPGA config: L span alpha type(0=RC,1=RRC)");
        Serial.println("Example: 8 40 0.35 1");
        waiting_for_input = true;
        input_mode = 'F';
        input_idx = 0;
        break;
      ...
      case 'b': {
        modulator->set_cic_bypass(!modulator->get_cic_bypass());
        Serial.print("RRC Filter Mode: ");
        Serial.println(modulator->get_cic_bypass() ? "BYPASSED (Square Wave)"
                                                   : "FILTERED (RRC)");
        break;
      }
      case 'q': {
        ...
        if (modulator->get_rrc_enabled()) {
          ...
          Serial.print(
              (float)(modulator->get_symbolrate() * modulator->get_rrc_L()) /
                  1000000.0f,
              3);
          Serial.println(" MHz)");
          Serial.println("  Interpolation Mode: ZOH Mode");
        } else {
          Serial.println("OFF");
        }
        ...
        if (modulator->get_ldpc_enabled()) {
          ...
          ep = (modulator->get_reed_solomon_enabled()
                    ? modulator->get_rs_input_size()
                    : modulator->get_rs_output_size()) -
               (modulator->get_fecf_enabled() ? 2 : 0) - 8;
        }
        Serial.print(ep);
        Serial.println(" bytes");
        ...
        if (headroom_pct < 0)
          Serial.println(
              "  [!] WARNING: CPU Too Slow! Pipeline Starvation Occurring!");
        break;
      }
      case 'x': {
      ...
      case 'C': {
        Serial.println(
            "Enter correction (0.01 Hz units, e.g. -150 for -1.5 Hz):");
        waiting_for_input = true;
        input_mode = 'C';
        input_idx = 0;
        break;
      ...
      case 'W': {
        if (modulator != nullptr) {
          ...
          modulator->update_rrc_config(!current, modulator->get_rrc_alpha(),
                                       modulator->get_rrc_filter_span(),
                                       modulator->get_rrc_min_dac_rate(),
                                       modulator->get_use_rrc());
